...
class ToadEnergyMeasurement:
    ...
    def measure_energy(self):
        ...
        for problem_file in self.problem_files:
            ...
            @pyRAPL.measureit(number=30, output=csv_output)
            def run_toad():
                print(f"\033[91m{'run_toad'}\033[0m")
                command = f"{self.toad} 'BU-IO' {ground_file} ht ipt=15"
                return self.run_command(command)

            @pyRAPL.measureit(number=30, output=csv_output)
            def run_downward():
                print(f"\033[91m{'run_downward'}\033[0m")
                command = f"{self.downward} --evaluator 'hff=ff()' --search 'lazy_greedy([hff], preferred=[hff])' < {problem_sas}"
                return self.run_command(command)
