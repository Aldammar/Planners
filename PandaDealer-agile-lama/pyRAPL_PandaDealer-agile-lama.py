    ...
    def measure_energy(self):
        ...
        for problem_file in self.problem_files:
            ...
            def run_pandaPIgrounder():
                ...
                command = f"{self.pandaPIgrounder} -q {parsed_file} {psas_file}"
                return self.run_command(command)

            @pyRAPL.measureit(number=30, output=csv_output)
            def run_pandaPIengine():
                print(f"\033[91m{'run_pandaPIengine'}\033[0m")
                command = f'{self.pandaPIengine} "--heuristic=lama(lazy=false;ha=false;lm=lmc;useLMOrd=false;h=add;search=gbfs)" {psas_file} >> {log_file}'
                return self.run_command(command)
